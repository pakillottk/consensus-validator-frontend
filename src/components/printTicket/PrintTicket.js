...
class PrintTicket extends React.Component {
  componentDidUpdate() {
    if( this.props.print ) {
      ...
      for( let i = 0; i < barcodes.length; i++ ) {
        ...
        const [w,h] = [barcodes[i].getAttribute("width"), barcodes[i].getAttribute("height")];
        if( w !== undefined && w !== null )
        {
          barcodes[ i ].setAttribute( "jsbarcode-width", w);
        }
        if( h !== undefined && h !== null )
        {
          barcodes[ i ].setAttribute( "jsbarcode-height", h);
        }
      }
  ...
  render() {
    ...
    const { TicketPrinter } = this.props
    const tickets = [];
    this.props.tickets.forEach(
      ( ticket ) => {
        tickets.push( TicketPrinter( ticket, this.props ) );
      }
    )
    return (
      <div>
